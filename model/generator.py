...
class GeneratorCell(nn.Module):
    ...
    def __init__(self, device, batch_size, n, m, embedding_layer):
        ...
        self.gru = nn.GRUCell(input_size=self.m, hidden_size=self.m, device=device)
    ...
    def forward(self, st1, ht1, zt, da_t):
        ...
        Ht1 = Mt1_2.squeeze(1)  # b*1*m -> b*m

        if DEBUG: print(f'zt: {zt.size()}, da_t: {da_t.size()}')
        xt = self.conv1d_xt(torch.cat((zt, da_t), dim=1).unsqueeze(1)).squeeze(1)  # b*1*n -> b*1*m -> b*m
        if DEBUG: print(f'xt: {xt.size()}, Ht1: {Ht1.size()}')
        ht = self.gru(xt, Ht1)  # 1*b*n
        ht = ht.squeeze(0)
